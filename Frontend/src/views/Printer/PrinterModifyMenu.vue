...
function submit() {
  ...
  }else if (brand.value !== "" && model.value === "") return;
  ...
  <div class="fixed top-0 left-0 fixHeight w-screen bg-black bg-opacity-25 z-30 flex items-center justify-center" @click.self="$emit('close')">
    <form v-on:submit.prevent="submit" class="bg-white p-6 rounded-3xl">
      ...
      <div class="flex justify-center">
        <button class="w-full bg-green-500 text-white p-2 rounded-lg enabled:cursor-pointer hover:bg-green-600 disabled:bg-green-700" type="submit" :disabled="loading || delloading">
        ...
        <button class="w-full bg-red-500 hover:bg-red-600 disabled:bg-red-700 enabled:cursor-pointer p-2 rounded-lg ml-1 text-white" @click="deletePrinter" :disabled="loading || delloading">
